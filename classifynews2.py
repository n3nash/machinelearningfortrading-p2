...
def testing_data(fileName):
	...
	totalnew = totalnew / divide
	for key,value in newwordList.items():
		sum = sum + (value*(weights[i]*float(value/totalnew)))
		if i>=len(weights)-1:
			break
		i=i+1
	global count
...
if __name__=="__main__":
	...
	weights = [0 for x in range(hashtable)]
	totalgood=0
	totalbad = 0
	totalcount = 0
	remove = []
